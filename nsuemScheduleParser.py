...
def getEventsList():
	# fetching table
	page = urllib2.urlopen("http://rasp.nsuem.ru/group/1761")
		...
		try:
			...
			while cell is not None:
				...
				try:
					...
					if cell.b is None:
						exercise['type'] = 'Л'
						cellData = getTDData(cell)
					else:
						exercise['type'] = 'С'
						cellData = getTDData(cell.b)
					exercise['name'] = cellData['name']
					exercise['room'] = cellData['room']
					returnData.append('-'.join(formCalEvent(exercise)))
				else:
...
def getTDData(scope):
	...
	returnData['name'] = name[0:len(name) - 8]
	returnData['room'] = data.a.get_text().encode('utf-8')
	return returnData
